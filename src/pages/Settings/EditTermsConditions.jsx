...
const EditTermsConditions = () => {
  ...
  const [terms, setTerms] = useState(content?.data?.termsAndConditions || "");

  const [updateTerms, { isLoading: updateLoading }] =
    useUpdateSettingsMutation();

  useEffect(() => {
    if (content?.data?.termsAndConditions) {
      setTerms(content.data.termsAndConditions);
    }
  }, [content]); // Runs when `content` updates

  const handleUpdate = async () => {
    try {
      const res = await updateTerms({ termsAndConditions: terms }).unwrap();
